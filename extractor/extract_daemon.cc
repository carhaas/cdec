...
GrammarExtractor read_data(po::variables_map &vm, ofstream &log_file){
  ...
  if(target_fstream.fail()){
    log_file << "Error: cannot find: " << vm["target"].as<string>() << endl;
    exit (EXIT_FAILURE);
  }
  ...
  if(source_fstream.fail()){
    log_file << "Error: cannot find: " << vm["source"].as<string>() << endl;
    exit (EXIT_FAILURE);
  }
  ...
  if(alignment_fstream.fail()){
    log_file << "Error: cannot find: " << vm["alignment"].as<string>() << endl;
    exit (EXIT_FAILURE);
  }
  ...
  if(precomputation_fstream.fail()){
    log_file << "Error: cannot find: " << vm["precomputation"].as<string>() << endl;
    exit (EXIT_FAILURE);
  }
  ...
  if(vocabulary_fstream.fail()){
    log_file << "Error: cannot find: " << vm["vocabulary"].as<string>() << endl;
    exit (EXIT_FAILURE);
  }
  ...
  if(ttable_fstream.fail()){
    log_file << "Error: cannot find: " << vm["ttable"].as<string>() << endl;
    exit (EXIT_FAILURE);
  }
...
string extract_for_sentence(GrammarExtractor &extractor, po::variables_map &vm, string sentence, int &i, bool use_zip, string path = ""){
	string set_grammar_path = vm["grammars"].as<string>();
	
	if(path!=""){
		set_grammar_path = path;
		i = 0;
	}
	
	fs::path grammar_path = set_grammar_path;
  ...
  while(1){
    ...
    assert (bytes >= 0);
		...
		if (position != message.npos) {
			sentence = message.substr(0, position);
			path = message.substr(position+7);
		} else {
			sentence = message;			
		}
    ...
    log_file << "Extracting sentence number " << count
         << " since the start of this daemony" << endl;

    string output_sentence = extract_for_sentence(extractor, vm, sentence, count, use_zip, path);
